"""
This is the passbrute module.
Needs macchanger and iproute2
Assumes wlan0
"""
...
def check_password(args):
    ...
    This function is partally AI generated, AI made this multi-core
...
def passbrute():
    ...
    class mac:
        # MAC changing made easy with classes and functions
        def rand():  # pyright: ignore[reportSelfClsParameterName]
            # Changes the MAC to a randomly generated one
        ...
        def perm(): # pyright: ignore[reportSelfClsParameterName]
            # Changes the MAC to the normal MAC
            os.system("sudo ip link set wlan0 down")
            time.sleep(0.002)
        ...
        def check(): # pyright: ignore[reportSelfClsParameterName]
            # Shows the current MAC
            os.system("macchanger --show wlan0")

    class net:
        # Easy way to bring up and down the Wireless NIC
    ...
    # Check if we have permission to change the MAC address
    ...
    # Needed as sometimes macchanger will say it failed when it didn't
    input("Just in-case, double-check your MAC address. Press any key to continue, ^C to abort...")
    time.sleep(1)

    # List avalibe networks 
    os.system("iwctl station wlan0 scan")

    # Pretend there's a rich loading thing
    typer.echo("Wait.")
    time.sleep(3)
   
    # Get networks avalibe
    os.system("iwctl station wlan0 get-networks")

    # Allow a user to select a network
    target_ssid = input("Type target SSID: ")
    time.sleep(1)
    # See if the user would like to use a wordlist 
    print("Type the wordlist name (none will do random from minimum 8 characters)")
    wordlist_name = input("Wordlist name (wordlists can be stored in src/modules/passbrute/word_lists/): ").strip()
    
    # Initialise variables
    use_brute_force = False
    # The passwords are stored as a list
    passwlist = []
    
    if not wordlist_name:
        # Empty input - use brute force
        # Searched this up with AI
